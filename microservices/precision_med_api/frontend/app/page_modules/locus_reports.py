...
def render_locus_reports(release: str, job_name: str, config: FrontendConfig):
    ...
    exomes_data = load_locus_report(release, job_name, "EXOMES", config.results_base_path)

    if not wgs_data and not nba_data and not imputed_data and not exomes_data:
        st.info(
            "**No locus report data available.**\n\n"
            "Locus reports require:\n"
            "- Genotype data (WGS, NBA, IMPUTED, or EXOMES)\n"
    ...
    data_type = st.selectbox(
        "Select data type:",
        available_data_types,
        help="Choose which genotype data source to view (WGS = whole genome sequencing, NBA = genotyping array, IMPUTED = imputed genotypes, EXOMES = clinical exomes)"
    )
    ...
    if total_metrics:
        with st.expander("📊 Total Metrics", expanded=False):
            ...
            with col1:
                ...
                st.metric("Disease Duration ≤ 3 years", f"{total_metrics.get('disease_duration_lte_3_years', 0):,}")
            with col2:
            ...
            with col4:
                st.metric("H&Y < 3", f"{total_metrics.get('hy_less_than_3', 0):,}")
