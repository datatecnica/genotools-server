...
| `{job}_variant_report.csv` | CSV | Per-sample clinician-facing carrier report (NBA + WGS) |
...
## 3. Variant Report CSV

**File:** `{job}_variant_report.csv`

Clinician-facing carrier report in long format: one row per carrier per variant. Only
NBA and WGS data are included. IMPUTED and EXOMES do not contribute rows.

Generated by `LocusReportGenerator.generate_variant_report()`. Controlled by
`--skip-variant-report`.
...
| Column | Type | Description |
|--------|------|-------------|
| `GP2ID` | str | Sample identifier |
| `Ancestry` | str | Genetic ancestry code (AAC, EUR, EAS, etc.) |
| `Gene` | str | Gene / locus name (e.g., GBA1, LRRK2) |
| `Variant_ID` | str | hg38 coordinate (`chr:pos:ref:alt`) |
| `AA_change` | str | Amino acid change / mutation name from SNP list (e.g., `p.Gly2019Ser`) |
| `rsID` | str | dbSNP identifier (if available) |
| `NBA_probe_name` | str | PGEN variant ID for NBA-sourced rows; empty for WGS-only rows |
| `Zygosity` | str | `het` (one copy) or `hom` (two copies) |
| `MOI` | str | Mode of inheritance from SNP list (e.g., `AD`, `AR`) |
| `Data_type` | str | `NBA`, `WGS`, or `NBA, WGS` when cross-validated |
| `Pathogenicity` | str | Pathogenicity classification from SNP list |
| `Pathogenicity_source` | str | Source of the pathogenicity classification |
| `Variant_interpretation` | str | Same as `Pathogenicity` (retained for report template compatibility) |
| `potential_comp_het` | bool | True if this sample carries ≥2 het variants in the same AR gene |
| `validated_by_wgs` | bool \| None | See below |
| `wgs_carrier_concordance` | float \| None | See below |

### `validated_by_wgs` — Three States

| Value | Meaning |
|-------|---------|
| `True` | NBA carrier confirmed: same sample is also a WGS carrier for this variant |
| `False` | NBA carrier not confirmed: sample is in the WGS cohort but WGS does not call this variant |
| `None` | Not applicable: either the sample has no WGS data, or this row is a WGS-only row |

### `wgs_carrier_concordance` — Per-Variant Concordance Rate

Float between 0.0 and 1.0. For each variant (`snp_list_id`): the fraction of NBA
carriers **who also have WGS data** that are confirmed as carriers by WGS.

Example: if 8 out of 10 NBA carriers for `p.Gly2019Ser` (who have WGS data) are also
called as carriers by WGS, `wgs_carrier_concordance = 0.8` for every row with that
`AA_change`.

`None` when no WGS data is available or no NBA carriers for this variant have WGS data.

### Deduplication

When a sample is a carrier in both NBA and WGS, a single row is kept (NBA row is
preferred, with `Data_type = "NBA, WGS"`). Only the best-performing NBA probe per
variant is included (via probe selection).
