# This script creates:
# K8S NS 
# Sets up a persistent volume and claim for GCP bucket in a GKE cluster.
#!/bin/bash
ZONE=europe-west4-a
REGION=europe-west4
...
KSA_ExternalSecret_apps=ksa-external-secrets
#####
...
echo -e "\033[32mset current name space $K8S_NAMESPACE_APPS for the cluster\033[0m"

kubectl config set-context --current --namespace=$K8S_NAMESPACE_APPS
...
######
# echo -e "\033[32mNow creating k8s service account: $KSA_Bucket in namespace: $K8S_NAMESPACE to access the buckets: $BUCKET_NAME and $BUCKET_NAME_APPS\033[0m"
...
echo -e "\033[32mCreate a KSA $KSA_ExternalSecret_apps and configure to have access to Secret Manage by binding it to the GCP SA using Workload Identity Federation for GKE.\033[0m"
...
kind: ServiceAccount
metadata:
  name: ${KSA_ExternalSecret_apps}
  namespace: ${K8S_NAMESPACE_APPS}
  annotations:
    iam.gke.io/gcp-service-account: ${GCP_SA}@${PROJECT_ID}.iam.gserviceaccount.com
EOF
echo -e "\033[32mBinding the k8s sa: $KSA_ExternalSecret_apps to impersonate as real gcp service account via $GCP_SA for secret manager access\033[0m"
gcloud iam service-accounts add-iam-policy-binding $GCP_SA@$PROJECT_ID.iam.gserviceaccount.com \
    --role roles/iam.workloadIdentityUser \
    --member "serviceAccount:$PROJECT_ID.svc.id.goog[$K8S_NAMESPACE_APPS/$KSA_ExternalSecret_apps]" 

echo  -e "\033[32mNow starting settings for pregenotools namespace\033[0m"

BUCKET_NAME_PREGENOTOOLS=gtserver-eu-west4-gp2-release-terra
PV=pv-pregenotools
PVC=pvc-pregenotools
#service accounts
#GCP SA
GCP_SA=gsa-prod-gtserver
...
echo -e "\033[32mSetting up a cloud storage bucket: $BUCKET_NAME_PREGENOTOOLS, PLEASE IGNORE IF bucket is already there\033[0m"

gcloud storage buckets create gs://$BUCKET_NAME_PREGENOTOOLS \
  --location $REGION --uniform-bucket-level-access --project $PROJECT_ID


echo -e "\033[32mSetting up IAM policy for the bucket: $BUCKET_NAME_PREGENOTOOLS and allow access via: $GCP_SA service account we created above\033[0m"
